    ...
    <div>
        ...
        <div className="info-box mb-3" style={{ padding: '0.5rem' }}>
          <div className="flex justify-between items-center mb-1">
            ...
            <div className="flex gap-3 items-center">
              {internalFdaBalance !== null && assetType === 'token' && tokenAddress.toLowerCase() === FDA_TOKEN_ADDRESS.toLowerCase() && (
      ...
      {assetType === 'token' && (
        <>
          <div className="mb-2">
            ...
            <div className="flex gap-2 mb-2">
              <button
                ...
                onClick={() => setTokenAddress('0x55d398326f99059fF775485246999027B3197955')}
              >
                USDT (BSC)
              </button>
              <button
          ...
          {tokenAddress.toLowerCase() === FDA_TOKEN_ADDRESS.toLowerCase() && auth ? (
            <div className="card-dark mb-2" style={{ padding: '0.5rem' }}>
              <label className="block text-sm text-white mb-1 font-semibold" style={{ color: '#f1f5f9' }}>Transfer Type:</label>
              <select
                className="form-select-dark text-xs py-1"
                value={transferType}
                onChange={(e) => setTransferType(e.target.value as 'internal' | 'onchain')}
              >
                <option value="internal">🔄 Internal Transfer (Zero Fee, Instant)</option>
                <option value="onchain">⛓️ On-Chain Transfer (Gas Fee Required)</option>
              </select>
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2 items-center">
          ...
          {allWallets.length > 1 && (
            <select
              ...
              onChange={(e) => {
                if (e.target.value) {
                  const wallet = allWallets.find((w) => w.id === e.target.value);
                  if (wallet) {
        ...
        {/* MC Wallet Detection */}
        {assetType === 'token' && tokenAddress.toLowerCase() === FDA_TOKEN_ADDRESS.toLowerCase() && sendTo.trim() && ethers.isAddress(sendTo.trim()) && (
          <>
            {recipientFdaWallet ? (
              <div className="success-box-dark mb-2" style={{ padding: '0.5rem' }}>
                <p className="text-xs text-slate-50 font-semibold mb-1">
                  ✅ MC Wallet Detected
                </p>
                <p className="text-xs text-slate-200">
                  {recipientFdaWallet.fullName || recipientFdaWallet.email || recipientFdaWallet.walletLabel || 'MC Wallet'}
                </p>
              ...
              <div className="warning-box mb-2" style={{ padding: '0.5rem' }}>
                ...
                <p className="text-xs text-slate-200 mb-2">
                  This address is not registered in the MC system. Register it to enable zero-fee internal transfers.
                </p>
                {auth && (
                  <button
        ...
        <div className="flex gap-2">
          ...
          {transferType === 'onchain' && (
            <button
              ...
              disabled={!unlockedPrivateKeyRef.current || !sendTo.trim() || estimatingGas}
      ...
      {transferType === 'onchain' && estimatedGas && unlockedPrivateKeyRef.current && (
        <div className="card-dark mb-3" style={{ padding: '0.5rem' }}>
          <div className="flex justify-between items-center">
            <p className="text-xs text-slate-400">Estimated Gas Fee:</p>
            <p className="text-xs text-slate-50 font-semibold">~{estimatedGas} BNB</p>
          </div>
          <p className="text-xs text-slate-500 mt-1" style={{ fontSize: '0.7rem' }}>
            ⚠️ Gas fees are required for all blockchain transactions and must be paid in BNB (native token).
            {assetType === 'native' && nativeBalance !== null && (
              <span className="block mt-1">
                You have {parseFloat(nativeBalance).toFixed(6)} BNB. 
                {parseFloat(sendAmount || '0') > 0 && (
                  <span> Total needed: {(parseFloat(sendAmount) + parseFloat(estimatedGas)).toFixed(6)} BNB</span>
                )}
              </span>
            )}
          </p>
